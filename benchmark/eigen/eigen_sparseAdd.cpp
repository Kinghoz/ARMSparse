//g++ -I ~/zhengj/work/eigen-3.3.9 compare.cpp -o compare -O3 -fopenmp
...
void bench_Sparse(const spMatFloat &m, const spMatFloat &in, spMatFloat &o) {
...
int main(int argc, const char **argv) {
  ...
  for(int index=0; index<iter; index++) {
    ...
    spMatFloat s_m_c = d_m.sparseView();
    for(int i=0;i<k;i++){
      for(int j=0;j<n;j++){
    ...
    // }
    {
      ...
      for(int k=0;k<repeat;k++) bench_Sparse(s_m,s_m_b,s_m_c);
      // clock_t end = clock();
